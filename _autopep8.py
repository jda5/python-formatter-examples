...
import sys
import os
import json  # multiple imports on one line to test linting
...
def multiple_args_function(
    ...
    return first + second + third + fourth + fifth + sixth + \
        seventh + eigth + ninth + (0 if flag is None else 1)
...
def create_users(names, starting_age=0) -> List[User]:
    ...
    """
    return [User(username=name, age=(idx + starting_age))
                ...
                case list() as list_val:
                    ...
                    if len(list_val) > 5:
                        self.internal_state[key] = [
                            elem * 2 for elem in list_val if isinstance(elem, (int, float))]
                    else:
...
def main():
    ...
    # Testing the existence of an OS environment variable (simple conditional
    # usage)
